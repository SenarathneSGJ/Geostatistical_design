...
for(i in 1:length(vt)){
  loc <- paste("Example1/Simulation_results/Pred_Ute_approximations/out",vt[i],".RData",sep="")
  load(loc)
  ute_mat <- matrix(out.data,ncol=4,byrow=T)
  pred_approx=c(pred_approx,ute_mat[,3])
  pred_act=c(pred_act,ute_mat[,4])
}
plot(pred_act,pred_approx,xlim=c(-200,-135),ylim=c(-190,-115),ylab=bquote("Loss values obtained from " ~~tilde(lambda)[P]),xlab=bquote("Loss values obtained from " ~hat(lambda)[P]),cex=0.5)
...
text(x = 0.5, y = 0.5, cor.X1X2[i], cex = 1.1, col = "black")
  ...
  for(j in 1:3){
    vec <- paste("Example1/selected_designs/",d.crit[j],"_",cor.val[i],"_D5.RData",sep="")
    load(vec)
...
for(i in 1:3){
  ...
  for(j in 1:3){
    vec <- paste("Example1/selected_designs/",d.crit[j],"_",cor.val[i],"_D10.RData",sep="")
    load(vec)
    plot(out.data[[1]],xlab="X1",ylab="X2",sub=loss_func[j],pch=19,
         xlim=c(0,1),ylim=c(0,1),col='blue')
  }
...
for(j in 1:2)
{
  for(k in 1:3){
    E.loss=c();P.loss=c()
    for(i in 1:N){
      vec<- paste("Example1/Simulation_results/Design_evaluation/",cor.val[k],"D",loc.d[j],"/out",i,".RData",sep="")
      load(vec)
...
p1 <- ggplot(Est_Loss, aes(x = factor(Design_points,level = c('n=5','n=10')), y = Expected_loss,fill=factor(Design,level=c('Estimation','Dual-purpose','Prediction'))))+
...
p2 <- ggplot(Pred_Loss, aes(x = factor(Design_points,level = c('n=5','n=10')), y = Expected_loss,fill=factor(Design,level=c('Estimation','Dual-purpose','Prediction'))))+
  geom_boxplot(outlier.shape = NA)+ facet_wrap(.~factor(Dependence,level=c("Week","Moderate","Strong")), scales = "free")
...
Estimation_loss=aggregate(Est_Loss$Expected_loss, list(Est_Loss$Design_points,Est_Loss$Dependence,Est_Loss$Design), FUN=mean) 
Estimation_loss <- Estimation_loss[order(Estimation_loss[,1],Estimation_loss[,2],Estimation_loss[,3],decreasing =T), ]

Prediction_loss=aggregate(Pred_Loss$Expected_loss, list(Pred_Loss$Design_points,Pred_Loss$Dependence,Pred_Loss$Design), FUN=mean) 
...
#Table 3
Data.loss=data.frame(n.points=Estimation_loss[,1],Dependece=Estimation_loss[,2],Design=Estimation_loss[,3],Est.efficiency,Pred.efficiency)
View(Data.loss)
