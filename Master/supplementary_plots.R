...
Monitoring_sites <- read.csv("Example2/Rcodes/design_selection/monitoring_sites_selected.csv")
...
for(i in 1:2){
  path.dE <- paste("Suplementary_results/C3_simulations/est_d",loc.d[i],".RData",sep="")
  load(path.dE)
  Est.Opt <- out[[2]]
  
  path.dB <- paste("Suplementary_results/C3_simulations/dual_d",loc.d[i],".RData",sep="")
  load(path.dB)
  EstnPred.Opt <- out[[2]]
  
  path.dP <- paste("Suplementary_results/C3_simulations/pred_d",loc.d[i],".RData",sep="")
  load(path.dP)
  Pred.Opt <- out[[2]]
  ...
  plot(opt_loc.B$Loc_X/10000,opt_loc.B$Loc_Y/10000,pch=18,cex.sub=0.8,cex.lab=0.8,cex.axis=0.8,col="green",cex=2.5,xlab="X-coordinate",ylab="Y-coordinate",sub=paste("(" ,pl_name[i],") C3,N=",loc.d[i],sep=""),
...
for(j in 1:2)
{
  ...
  for(i in 1:n1){
    vec<- paste("Suplementary_results/C3_simulations/d",loc.d[j],"/out",i,".RData",sep="")
    load(vec)
...
# Figure S2
p1 <- ggplot(Est_Loss, aes(x = factor(Design_points,level = c('n=5','n=7')), y = Expected_loss,fill=factor(Design,level=c('Estimation','Dual-purpose','Prediction'))))+
...
Ute.Approx2=c()
for(i in 1:50){
  d.name=paste("Suplementary_results/Efficiency_prediction_loss_approximations/Example1/out",i,".RData",sep="")
  load(d.name)
...
Ute.Approx2=c()
for(i in 1:50){
  d.name=paste("Suplementary_results/Efficiency_prediction_loss_approximations/Example2/out",i,".RData",sep="")
  load(d.name)
...
# Figure S7
load("Suplementary_results/Compare_common_designs/Post_pred_var_Y1.RData")
...
# Figure S8 
load("Suplementary_results/Compare_common_designs/Post_pred_var_Y2.RData")
...
# Figure S9
load("Suplementary_results/Compare_common_designs/Post_var.RData")
...
for(i in 1:6){
  ...
  p[[i]]=ggplot(data2, aes(x=prior, y=post)) +
    geom_point(size=2,aes(shape=parameter,col=parameter)) + ylim(0,1.75)+scale_shape_manual(values=1:nlevels(parameter)) +
