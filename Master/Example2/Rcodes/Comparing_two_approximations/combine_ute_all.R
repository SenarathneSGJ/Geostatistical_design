combine_ute_all <- function(d,X,Y,theta,distM){
  ...
  if(!is.symmetric.matrix(hessian1)){
      ...
      if (!is.positive.semi.definite(hessian1, tol=1e-16) | det(hessian1)==0) {
        ...
        t1=proc.time()
        pred.out <- Pred_ute.App(X_unsamp=X_unsamp,distM=distM_uns,post_samp1=post_samp,r01=r01,r02=r02)
		    t2=proc.time()
		    time1= t2-t1
		    
		    post_samp2 <-  rmvnorm(100,mu_post,Sigma_post)
		    t3=proc.time()
		    pred.out2 <- Pred_ute(X_unsamp=X_unsamp,distM=distM_uns,post_samp1=post_samp,post_samp2=post_samp2,r01=r01,r02=r02)
		    t4=proc.time()
		    time2= t4-t3
