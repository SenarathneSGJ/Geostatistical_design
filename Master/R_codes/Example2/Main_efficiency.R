...
for(j in 1:20){
  Y <- Res_Clcpp(xdata=X_data,distM=distM,para=theta_int[j,],Z1=t(Z_res[1:nrow(X_data),1]),Z2=t(Z_res[1:nrow(X_data),2]),
		Z3=Z_res[1:nrow(X_data),3],v2=Unif_set[1:nrow(X_data)],r01=r01,r02=r02) 
  crit.out <- combine_ute_all(d=d,X=X_data,Y=Y,theta=theta_int[j,],distM=distM)
  v_name <- paste("Results/Suplementary_results/Efficiency_prediction_loss_approximations/Example2/out",j,".RData",sep="")
