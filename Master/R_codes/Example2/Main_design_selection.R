...
# meteorological data at the prediction locations
data2017 <- read.csv("Data/Meteorological_data2017.csv") 
...
for(k in 1:10){
  for(i in 1:d_no){
    ...
    ute <- foreach(j=1:nrow(Unsamp_new),.packages=c("mvtnorm","corpcor",
            "nlme","Matrix","matrixcalc","AirQualityRcpp","Rcpp",
            "RcppArmadillo"),.verbose=TRUE,.combine=c) %dopar% 
    {
      d_new <- d_prop    
...
f.name <- paste("Results/Example2/Selected_design/",Des[utility],"_d",d_no,".RData",sep="")
