...
for(j in 1:15){
Y <- Res_Clcpp(xdata=as.matrix(X),distM=distM,para=theta_int[j,],Z1=t(Z_res[1:nrow(X),1]),
		Z2=t(Z_res[1:nrow(X),2]),Z3=Z_res[1:nrow(X),3],v2=Unif_set[1:nrow(X)],r01=r01,r02=r02)
crit.out <- combine_ute_both(d,X,Y,theta=theta_int[j,],distM)

v_name <- paste("Results/Suplementary_results/Efficiency_prediction_loss_approximations/Example1/out",j,".RData",sep="")
